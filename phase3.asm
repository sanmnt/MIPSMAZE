...
branchPoint: #if out of moves find a random place to start 
	lw $t3, wid #load width
	lw $t4, hgt #load height
	addi $t4, $t4, -1 #decrease height by one 
	jal rand #get random number
	div $v0, $t3 #divide random number by width
	mfhi $t3 #load remainder into t3
	addi $t3, $t3, 1 #add by 1 to push it one to the right
	sw $t3, cx #load number to cx
	jal rand #get another random number
	div $v0, $t4 #divide number by height
	mfhi $t4 #load remainder in t4
	addi $t4, $t4, 1 #add 1 to push the number by 1
	sw $t4, cy #load number to cy
	move $ra, $s1 #move the $ra to $s1 for future use
	jr $ra #jump to return address
checkIfinMaze: #make sure starting position is in maze
	lb $t8, board($s0) #load value into t8
	addi $t8, $t8, -48 #convert to ascii
	bgt $t8, 4, branchPoint #if greater than 4 go to branchpoint
	blt, $t8, 1, branchPoint #if less than 1 go to branchpoint
	jr $ra #jump to return address
